...
void USProbeObjectRepresentation::CreateActors()
{
  ...
  this->imageCubeActor = vtkActor::New();
  ...
  this->imageCubeActor->GetProperty()->SetColor(0.5,0.5,0.67);
  this->imageCubeActor->GetProperty()->SetOpacity(this->GetOpacity());

  imageCubeMapper->SetInputConnection(imageCube->GetOutputPort());
  this->imageCubeActor->SetMapper( imageCubeMapper );
...
void USProbeObjectRepresentation::setImagePlaneColor(float r, float g, float b)
{
    this->imageCubeActor->GetProperty()->SetColor(r,g,b);
}
